...
// TestWriteCopecCanvasData regenerates embedded canvas DATA when WRITE_COPEC_CANVAS=1.
func TestWriteCopecCanvasData(t *testing.T) {
	if os.Getenv("WRITE_COPEC_CANVAS") == "" {
		t.Skip("set WRITE_COPEC_CANVAS=1 to regenerate Copec canvas data")
	}

	root := os.Getenv("COPEC_EXPORT_ROOT")
	if root == "" {
		root = "/home/fmeneses/Documents/copec/export"
	}
	...
	outPath := os.Getenv("COPEC_CANVAS_DATA_OUT")
	if outPath == "" {
		outPath = "/tmp/copec-canvas-data.json"
	}
